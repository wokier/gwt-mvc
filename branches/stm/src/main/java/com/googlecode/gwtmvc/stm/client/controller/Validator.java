...
/**
 * This Validator performs string validation based on RegExp. It sets
 * {@link BooleanModel} to <code>TRUE</code> is string matches provided regular
 * expression.
 * 
 * NOTE: Validator is a widget and is active only when attached to DOM. When
 * detached validator removes itself from observing target string model.
 * 
 * @author Igor
 * 
 */
public class Validator extends Widget implements Listener<String> {

	private final BooleanModel valid;
	private final StringModel text;
	private final String regex;

	public Validator(BooleanModel visible, StringModel text, String regex) {
